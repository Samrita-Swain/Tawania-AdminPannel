...
export function UserActivityList({ activities }: UserActivityListProps) {
  ...
  const getActivityLink = (activity: Activity) => {
    switch (activity.type) {
      case 'SALE':
        return `/sales/${activity.related_id}`;
      case 'TRANSFER':
        return `/transfers/${activity.related_id}`;
      case 'INVENTORY':
        return `/inventory/${activity.related_id}`;
      default:
        return '#';
    }
  };
  ...
  return (
    <div className="space-y-4">
      {activities.map((activity, index) => (
        <div key={`${activity.type}-${activity.related_id}-${index}`} className="flex items-start space-x-4">
